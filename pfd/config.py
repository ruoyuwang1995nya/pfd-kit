...
class Config:
    ...
    inference_image_name: str = "registry.dp.tech/dptech/deepmd-kit:2024Q1-d23cf3e"
    ...
    def __post_init__(self):
        ...
        if not (self.context_type or self.machine):
        ...
        elif self.context_type in ["SSHContext", "sshcontext", "SSH", "ssh"]:
            self.machine_dict = {
                "batch_type": self.batch_type,
                "context_type": self.context_type,
            ...
            if self.machine:
                update_dict(self.machine_dict, self.machine)
        elif self.context_type in ["LocalContext", "localcontext" "Local", "local"]:
            self.machine_dict = {
                "batch_type": self.batch_type,
                "context_type": self.context_type,
                "local_root": self.local_root,
    ...
    def mongodb_config_dict(self):
        mongodb_config = {"host": self.mongodb_host, "port": self.mongodb_port}
    ...
    def basic_config_dict(self):
        basic_config = {
            ...
            "inference_image_name": self.inference_image_name,
        }
        return basic_config
    ...
    def get_executor(self, dispatcher_config: dict) -> DispatcherExecutor:
        if not (self.context_type or self.machine):
            executor = None
        else:
