from distill.exploration.inference.eval_model import (
    EvalModel
)
from distill.exploration.inference.util import (
    get_mae,
    get_rmse
)
...
class DPTest(EvalModel):
    ...
    def evaluate(
        ...
        **kwargs): 
        ...
        np.savetxt(str(prefix / (name + ".energy.txt")),
            np.column_stack((orig_labeled_e, new_labeled_e)),
            fmt='%.6f')
        np.savetxt(str(prefix / (name + ".energy_per_atom.txt")), 
        ...
        np.savetxt(str(prefix / (name + ".force.txt")), 
        ...
        if self._data.has_virial():
            ...
            np.savetxt(str(prefix / (name + ".virial.txt")), 
                        np.column_stack((orig_labeled_f, new_labeled_f)), 
                        fmt='%.6f')
